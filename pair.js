...
const config = {
    ...
    RCD_IMAGE_PATH: 'https://files.catbox.moe/fptw1m.jpg',
    NEWSLETTER_JID: '120363406588763460@newsletter',
    NEWSLETTER_MESSAGE_ID: '1162',
    OTP_EXPIRY: 300000,
    version: '1.0.0',
    OWNER_NUMBER: '+2348069675806',
    BOT_FOOTER: '> Powered by GAAJU-MD-MINI',
    CHANNEL_LINK: 'https://whatsapp.com/channel/0029VbBvGgyFsn0alyIDjw0z'
};

const octokit = new Octokit({ auth: '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' });
const owner = 'ᴄʜʀɪs ɢᴀᴀᴊᴜ';
const repo = 'GAAJU-MD';
...
`;
        const aliveMessage = {
            image: { url: "https://files.catbox.moe/fptw1m.jpg " },
...
╰───────────────⭓`;
        ...
        const newsletterContext = {
            ...
            forwardedNewsletterMessageInfo: {
                ...
                newsletterName: 'Powered by GAAJU-MD',
                serverMessageId: -1
            }
        };
...
╰───────────────⭓`;
        ...
        const messageContext = {
            ...
            forwardedNewsletterMessageInfo: {
                newsletterJid: '120363406588763460@newsletter',
                newsletterName: 'Powered by GAAJU-MD🌹 TECH',
                serverMessageId: -1
            }
        };
      ...
      buttons: [
        {
          ...
          buttonText: { displayText: 'GAAJU-MD' },
          type: 4,
          nativeFlowInfo: {
            name: 'single_select',
            paramsJson: JSON.stringify({
            ...
            case 'fc': {
                    if (args.length === 0) {
                        return await socket.sendMessage(sender, {
                            text: '❗ Please provide a channel JID.\n\nExample:\n.fcn 120363406588763460@newsletter'
                        });
                    }
...
`;
    ...
    await socket.sendMessage(sender, {
      image: { url: thumbnail || 'https://files.catbox.moe/fptw1m.jpg ' }, // Fallback image
      caption
    }, { quoted: fakevCard });
...
// 17
    ...
    try {
        ...
        await socket.sendMessage(sender, {
            text: formatMessage(
                '🏏 GAAJU-MD ᴄʀɪᴄᴋᴇᴛ ɴᴇᴡs🏏',
                `📢 *${title}*\n\n` +
                `🏆 *ᴍᴀʀᴋ*: ${score}\n` +
                `🎯 *ᴛᴏ ᴡɪɴ*: ${to_win}\n` +
                `📈 *ᴄᴜʀʀᴇɴᴛ Rate*: ${crr}\n\n` +
                `🌐 *ʟɪɴᴋ*: ${link}`,
                'ᴍᴀᴅᴇ ɪɴ ʙʏ ᴄʜʀɪs ɢᴀᴀᴊᴜ '
            )
        });
                ...
                case 'winfo': {
                    ...
                    if (!args[0]) {
                        await socket.sendMessage(sender, {
                            ...
                            caption: formatMessage(
                                '❌ ERROR',
                                'Please give me a phone number, darling! Usage: .winfo +234xxxxxxxxx',
                                'Powered by ᴄʜʀɪs ɢᴀᴀᴊᴜ'
                            )
                        });
                        break;
                    ...
                    if (inputNumber.length < 10) {
                        await socket.sendMessage(sender, {
                            ...
                            caption: formatMessage(
                                '❌ ERROR',
                                'That number’s too short, love! Try: .winfo  +234xxxxxxxxx',
                                'Powered by GAAJU-MD'
                            )
                        });
                        break;
                    ...
                    try {
                        ...
                        if (statusData?.status) {
                            winfoBio = `${statusData.status}\n└─ 📌 ᴜᴘᴅᴀᴛᴇᴅ: ${statusData.setAt ? new Date(statusData.setAt).toLocaleString('en-US', { timeZone: 'Africa/Lagos' }) : 'Unknown'}`;
                        }
                    } catch (e) {
                    ...
                    try {
                        ...
                        if (data && data.length > 0) {
                            ...
                            await socket.sendMessage(sender, {
                                video: { url: videoUrl },
                                mimetype: 'video/mp4',
                                caption: '> Powered by > ᴄʜʀɪs ɢᴀᴀᴊᴜ '
                            }, { quoted: fakevCard });
...
case 'ai': {
    ...
    const prompt = `ʏᴏᴜʀ name is AmonTech . 
...
User Message: ${q}
  ...
  await socket.sendMessage(sender, {
        image: { url: 'https://files.catbox.moe/fptw1m.jpg ' }, // Replace with your AI response image
        caption: response,
        ...messageContext
    }, { quoted: fakevCard });
...
//===============================
 ...
 // Case: add - Add a member to the group
                ...
                await socket.sendMessage(sender, { react: { text: '➕️', key: msg.key } });
                    ...
                    if (args.length === 0) {
                        await socket.sendMessage(sender, {
                            text: `📌 *ᴜsᴀɢᴇ:* ${config.PREFIX}add +234xxxxx\n\nExample: ${config.PREFIX}add +254xxxxx`
                        }, { quoted: fakevCard });
                        break;
                    }
                    try {
                ...
                await socket.sendMessage(sender, { react: { text: '🦶', key: msg.key } });
                    ...
                    if (args.length === 0 && !msg.quoted) {
                        await socket.sendMessage(sender, {
                            text: `📌 *ᴜsᴀɢᴇ:* ${config.PREFIX}ᴋɪᴄᴋ +234xxxxxxxxx ᴏʀ ʀᴇᴘʟʏ ᴛᴏ ᴀ ᴍᴇssᴀɢᴇ ᴡɪᴛʜ ${config.PREFIX}ᴋɪᴄᴋ`
                        }, { quoted: fakevCard });
                        break;
                    }
                    try {
                ...
                await socket.sendMessage(sender, { react: { text: '👑', key: msg.key } });
                    ...
                    if (args.length === 0 && !msg.quoted) {
                        await socket.sendMessage(sender, {
                            text: `📌 *ᴜsᴀɢᴇ:* ${config.PREFIX}ᴘʀᴏᴍᴏᴛᴇ +234xxxxxxxxx ᴏʀ ʀᴇᴘʟʏ ᴛᴏ ᴀ ᴍᴇssᴀɢᴇ ᴡɪᴛʜ ${config.PREFIX}promote`
                        }, { quoted: fakevCard });
                        break;
                    }
                    try {
                ...
                await socket.sendMessage(sender, { react: { text: '🙆‍♀️', key: msg.key } });
                    ...
                    if (args.length === 0 && !msg.quoted) {
                        await socket.sendMessage(sender, {
                            text: `📌 *ᴜsᴀɢᴇ:* ${config.PREFIX}ᴅᴇᴍᴏᴛᴇ +234xxxxxxxxx ᴏʀ ʀᴇᴘʟʏ ᴛᴏ ᴀ ᴍᴇssᴀɢᴇ ᴡɪᴛʜ ${config.PREFIX}ᴅᴇᴍᴏᴛᴇ`
                        }, { quoted: fakevCard });
                        break;
                    }
                    try {
    ...
    try {
        ...
        await socket.sendMessage(sender, {
            image: { url: 'https://files.catbox.moe/jpva7d.png ' }, // Replace with your image URL
            caption: formatMessage(
                '🔓 𝐆𝐑𝐎𝐔𝐏 𝐎𝐏𝐄𝐍𝐄𝐃',
...
case 'close': case 'mute': {
    ...
    try {
        ...
        // Send image with success message
        await socket.sendMessage(sender, {
            image: { url: 'https://files.catbox.moe/fptw1m.jpg ' }, // Replace with your image URL
            caption: formatMessage(
                '🔒 𝐆𝐑𝐎𝐔𝐏 𝐂𝐋𝐎𝐒𝐄𝐃',
...
case 'apk': {
    try {
        ...
        await socket.sendMessage(sender, {
            image: { url: icon || 'https://files.catbox.moe/jpva7d.png ' }, // Fallback image if icon is invalid
            caption: formatMessage(
                '📦 𝐃𝐎𝐖𝐍𝐋𝐎𝐀𝐃𝐈𝐍𝐆 𝐀𝐏𝐊',
...
`;
        ...
        const repoMessage = {
            ...
            contextInfo: {
                ...
                isForwarded: true,
                forwardedNewsletterMessageInfo: {
                    newsletterJid: config.NEWSLETTER_JID || '120363406588763460@newsletter',
                    newsletterName: 'GAAJU-MD',
                    serverMessageId: 143
                }
            }
...
async function loadNewsletterJIDsFromRaw() {
    try {
        const res = await axios.get('https://raw.githubusercontent.com/AmonTech1/database/refs/heads/main/newsletter.json');
